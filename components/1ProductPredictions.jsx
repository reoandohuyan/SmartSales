...
try {
  ...
  if (!resPred.ok) {
    ...
    console.error("Products fetch failed:", resPred.status, text);
  ...
  console.log("Fetched products JSON:", prodJson); // debug

  if (prodJson && Array.isArray(prodJson.predictions)) {
    setPredictions(prodJson.predictions);
  } else {
    console.warn("No predictions found:", prodJson);
    setPredictions([]);
    setError("No product predictions available.");
  }
} catch (err) {
      ...
      return (
        <div className="space-y-4 w-full">
          ...
          {selectedProduct.stock < 20 && (
            <p className="text-red-400 font-semibold">Low stock! Consider restocking soon.</p>
          )}
          <div className="h-64">
            <Bar
              data={{
...
const handleDelete = async (productName) => {
  try {
    // Use query parameter instead of JSON body for DELETE
    const res = await fetch(
      `https://smartsales-dt0f.onrender.com/api/delete-product?product=${encodeURIComponent(productName)}`,
      {
        method: "DELETE",
      }
    );
    ...
    // Remove product from table without refetching
    setPredictions((prev) =>
      prev.filter((item) => item.product.toLowerCase() !== productName.toLowerCase())
    );

    alert(data.message || `Product "${productName}" deleted successfully.`);
    return true;
  } catch (err) {
    console.error(err);
  ...
  const openDeleteModal = (productName) => setDeleteModalProduct(productName);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-slate-200 font-sans">
      ...
      <div className="p-5 pt-32 space-y-5 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div onClick={() => openModal("line")} className="cursor-pointer bg-slate-800 border border-slate-700 rounded-xl shadow-lg p-5 hover:shadow-[0_0_25px_rgba(0,255,255,0.7)] transition-shadow duration-300">
          ...
          <div onClick={() => openModal("bar")} className="cursor-pointer bg-slate-800 border border-slate-700 rounded-xl shadow-lg p-5 hover:shadow-[0_0_25px_rgba(0,255,255,0.7)] transition-shadow duration-300">
            <h2 className="text-cyan-400 font-semibold mb-3 drop-shadow-[0_0_6px_#0ff] flex items-center gap-2">
          ...
          <div onClick={() => openModal("doughnut")} className="cursor-pointer bg-slate-800 border border-slate-700 rounded-xl shadow-lg p-5 hover:shadow-[0_0_25px_rgba(0,255,255,0.7)] transition-shadow duration-300">
            <h2 className="text-cyan-400 font-semibold mb-3 drop-shadow-[0_0_6px_#0ff] flex items-center gap-2">
        ...
        {/* Product Table */}
        <div className="overflow-x-auto mt-5 bg-slate-800 border border-slate-700 rounded-xl p-4 shadow-lg">
          <table className="w-full text-left text-slate-200 border-collapse">
            <thead>
              <tr className="border-b border-slate-600">
                <th className="p-2 cursor-pointer" onClick={() => requestSort("product")}>Product</th>
                <th className="p-2 cursor-pointer" onClick={() => requestSort("last_sales")}>Last Sales</th>
                <th className="p-2 cursor-pointer" onClick={() => requestSort("forecast")}>Forecast</th>
                <th className="p-2 cursor-pointer" onClick={() => requestSort("trend")}>Trend</th>
                <th className="p-2 cursor-pointer" onClick={() => requestSort("stock")}>Stock</th>
              ...
              {sortedAndFiltered.map((item, idx) => (
                <tr
                  ...
                  className={`border-b border-slate-700 hover:bg-slate-700 cursor-pointer ${
                    highlightProduct === item.product ? "bg-cyan-900/40" : ""
                  }`}
                  onClick={() => openModal(null, item)}
                >
                  <td className="p-2">{item.product}</td>
                  <td className="p-2">{item.last_sales}</td>
                  <td className="p-2">{item.forecast}</td>
                  <td className="p-2">{item.trend}</td>
                  <td className="p-2">{item.stock}</td>
                  <td className="p-2 flex gap-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); openDeleteModal(item.product); }}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-900 p-6 rounded-xl w-11/12 md:w-4/5 max-w-4xl">
            <button
  ...
  onClick={async () => {
    const success = await handleDelete(deleteModalProduct); // <- use state
