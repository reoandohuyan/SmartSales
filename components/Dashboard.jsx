...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (!Array.isArray(data) || data.length === 0) throw new Error("No sales data available.");

      const monthsArray = data.map((item) => item.month || "Unknown");
      const salesArray = data.map((item) => (typeof item.sales === "number" ? item.sales : 0));

      setMonths(monthsArray);
      setOriginalMonths(monthsArray); // store original disk values
      setSales(salesArray);
  ...
  const handleUpdateAll = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        alert("All sales updated successfully!");
      }
    } catch (error) {
  ...
  // --- DELETE single month ---
  const handleDelete = async (monthToDelete) => {
    if (!window.confirm(`Are you sure you want to delete sales for ${monthToDelete}?`)) return;

    try {
      const res = await fetch("http://localhost:5000/api/delete-sale", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ month: monthToDelete }),
      });
      ...
      if (!res.ok) {
        ...
        await fetchDashboardData();
        alert(`Deleted sales for ${monthToDelete}`);
      }
    } catch (err) {
      console.error("Error deleting sales:", err);
      alert("Error deleting sales. See console for details.");
    }
  };
        ...
        <div className="flex flex-col md:flex-row gap-5">
          ...
          <div className="flex-1 bg-slate-800 p-5 rounded-xl border border-slate-700 shadow-lg">
                  ...
                  {months.map((month, idx) => (
                    ...
                    >
                      ...
                      <td className="p-2 border-b border-slate-600 text-center">
                        <button
                          className="bg-red-500 px-3 py-1 rounded hover:scale-105 transition transform duration-200 text-white"
                          onClick={() => handleDelete(months[idx])}
                        >
                          Delete
                        </button>
                      </td>
            ...
            <div className="mt-3 flex gap-3 flex-wrap">
              ...
              <button
                className="bg-green-500 text-white px-4 py-2 rounded-lg hover:scale-105 transition transform duration-300"
